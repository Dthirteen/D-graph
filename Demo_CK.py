...
for (FLAG,curr_train_ratio) in [(7, 0.01)]:
    OA_ALL = []
    AA_ALL = []
    KPP_ALL = []
    AVG_ALL = []
    ...
    if FLAG == 6:
        data_mat = sio.loadmat('/home/shsun/Dingwenda/Hyperspectral_Image_Analysis_Simplified/Notebooks/Botswana.mat')
        data = data_mat['Botswana']
        gt_mat = sio.loadmat('/home/shsun/Dingwenda/Hyperspectral_Image_Analysis_Simplified/Notebooks/Botswana_gt.mat')
        ...
        dataset_name = "Botswana_SSCDenseNet"  # 数据集名称
        pass
    

    if FLAG == 7:

        data = h5py.File('/home/shsun/Dingwenda/Dataset/Chikusei_MATLAB/HyperspecVNIR_Chikusei_20140729.mat')['chikusei'][:].transpose()
        gt = sio.loadmat('/home/shsun/Dingwenda/Dataset/Chikusei_MATLAB/HyperspecVNIR_Chikusei_20140729_Ground_Truth.mat')['GT'][0,0]['gt']
        # 参数预设
        train_ratio = 0.01  # 训练集比例。注意，训练集为按照‘每类’随机选取
        val_ratio = 0.01 # 测试集比例.注意，验证集选取为从测试集整体随机选取，非按照每类
        class_count = 19  # 样本类别数
        learning_rate = 6e-4 # 学习率
        max_epoch = 1000  # 迭代次数
        weight_decay = 2e-5
        split_height = 20
    ...
    logger = get_logger('/home/shsun/Dingwenda/ENL-FCN/Demo_CK.log')
    exp_num=2
    '''
    for thre in np.arange(0.0055,0.006,0.0001):
        ...
        for num in range(exp_num):
            model = CCCon2B.SSCDNonLModel_ab(class_count, n_bands, 200) # Criss Cross Model CCNet 2B parallel
            #model = CCCon2B.SSCDNonLModel_gat(class_count, n_bands, 200)
            print(model)
...
'''
    for num in range(2):
        model = CCCon2B.SSCDNonLModel_ablation(class_count, n_bands, 200) # Criss Cross Model CCNet 2B parallel
        #model = CCCon2B.SSCDNonLModel_gat(class_count, n_bands, 200)
        print(model)
        if use_cuda: torch.backends.cudnn.benchmark = True
        if use_cuda: model.cuda()
