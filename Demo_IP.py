...
for (FLAG,curr_train_ratio) in [(1, 0.1)]:
    ...
    threshold=0.001
    #model = CCCon2B.SSCDNonLModel_gcns(class_count, 200, 150) # Criss Cross Model CCNet 2B parallel
    ...
    logger = get_logger('./Demo_IP.log')
    epoch=1000
    exp_num=10
    best=[]
    output_file=[]
    threshold=0.001
    model_names=[CCCon2B.SSCDNonLModel_ngcn_1,CCCon2B.SSCDNonLModel_gcn3,CCCon2B.SSCDNonLModel_ngcn_3]
    for name in model_names:
        model_name =name
        for num in range(10):
            model = model_name(class_count, n_bands, 200,threshold) 
            # Criss Cross Model CCNet 2B parallel
        ...
        #for num in range(exp_num):
            parm=tw.model_stats(model,[1,200,145,145])
            #print(parm)
            output_best=0
            best_acc = -1
            ...
            for eep in range(epoch):
                ...
                if eep==400:
                    ...
                    #break
            logger.info('model {}  best_acc {:.7f} threshold={} parm={} \n\n\n'.format(model,best_acc,threshold,parm))
